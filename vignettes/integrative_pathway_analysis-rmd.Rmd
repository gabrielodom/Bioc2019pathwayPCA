...
* Gabriel J. Odom^[University of Miami's Miller School of Medicine, Miami, FL] (<gabriel.odom@med.miami.edu>)
* Yuguang James Ban^[University of Miami's Sylvester Comprehensive Cancer Center, Miami, FL] (<Yuguang.ban@med.miami.edu>)
...
We also strongly recommend the [RStudio](https://www.rstudio.com/products/rstudio/download/) integrated development environment as a user-friendly graphical wrapper for R.
...
* [`survival`]() is the core CRAN package for survival analysis and censored data structures
...
* [`tidyverse` package suite](https://www.tidyverse.org/) is a collection of utility packages for data science: . (We will make use of some data constructs and ideas from these packages, but we do not expect users to be intimately familiar with Tidyverse)
...
- Let $\textbf{X} \in \mathbb{R}_{n \times p}$ be the observed single-omics data (gene expression, protein expression, copy-number variation, etc.), where subjects are recorded in the rows and -ome features are recorded in the columns (as per the Tidy Data design of [Wickham, 2014](https://doi.org/10.18637/jss.v059.i10)).
...
- Consider a column-subset of $\textbf{X}$, $\textbf{X}_k \in \mathbb{R}_{n \times p_k}$, that contains all of the sample values recorded for the -omes in pathway $k$.
- Let $\textbf{P}^{(k)}_d \in \mathbb{R}_{p_k \times d}$, for $d < p$, be the projection matrix of the $d$ eigenvectors corresponding to the largest $d$ eigenvalues of $\textbf{X}_k$. (Often we take $d = 1$, because a biological pathway should--in theory--only have one source of defined variability.)
- The principal components matrix for pathway $k$, $\textbf{Y}_k = \textbf{X}_k\textbf{P}^{(k)}_d \in \mathbb{R}_{n \times d}$, is the transformation of the original $p_k$ features in pathway $k$ into the $d$-dimensional orthogonal subspace that preserves the most information.
- Instead of modelling on *all* the recorded -ome values for a given pathway, we instead model on the few orthogonal PCs extracted from the pathway-specific data subsets.
...
AES-PCA extracts principal components from pathway $k$ which minimize this composite objective function
