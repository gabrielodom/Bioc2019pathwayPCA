...
What makes `pathwayPCA` different from other tools?

- `pathwayPCA` does not pre-filter features before analysis, unlike DAVID,^[<https://david.ncifcrf.gov/>] IPA,^[<https://www.qiagenbioinformatics.com/products/ingenuity-pathway-analysis/>] Enrichr,^[<http://amp.pharm.mssm.edu/Enrichr/>] `goseq`,^[<https://bioconductor.org/packages/release/bioc/html/goseq.html>] and other over-representative analysis methods.
...
- Other tools, such as `MOFA`,^[<https://bioconductor.org/packages/devel/bioc/html/MOFA.html>] `mixOmics`,^[<https://www.bioconductor.org/packages/release/bioc/html/mixOmics.html>] `mogsa`^[<https://bioconductor.org/packages/release/bioc/html/mogsa.html>], incorporate biological pathways *after* analysis is performed (for instance, PCs are extracted from the entire data set rather than a pathway-subset).
    + This is figuratively like putting a holiday feast into a blender and trying to taste the individual dishes: the PCs will extract global -omic signals while drowning out the effects of individual biological pathways.
...
The results showed there are 32 pathways significantly associated with survival in both CNV and protein data (*p*-value = 0.02088; Fisher's Exact Test). Here are the top-10 most significant pathways (sorted by protein data significance): 
```{r intersectPaths}
kable(
  SigBoth_df
)
...
Similar to the protein pathway analysis shown in [Section 5.3.4](#extr_rel_genes), we can also identify relevant genes with nonzero loadings that drives pathway significance in CNV. The "IL-1 signaling pathway" (WP195) is significant in both CNV and protein data, so we look for genes with non-zero loadings in both.
```{r wp195}
