...
There is a built version of this workshop available at <http://rpubs.com/gabrielodom/Bioc2019_pathwayPCA>. The source is available at <https://github.com/gabrielodom/Bioc2019pathwayPCA>.
...
* `pathwayPCA`,^[<https://gabrielodom.github.io/pathwayPCA/>] and the
...
`pathwayPCA` is an integrative analysis tool that implements the principal component analysis (PCA) based pathway analysis approaches.^[ [Chen et al. (2008)](https://doi.org/10.1093/bioinformatics/btn458) ] ^[ [Chen et al. (2010)](https://doi.org/10.1002/gepi.20532) ] ^[ [Chen (2011)](https://doi.org/10.2202/1544-6115.1697) ]
...
## Main Functions
`pathwayPCA` uses three main groups of functions: object creation, object analysis, and analysis inspection.

- `CreateOmics()` creates the data object with a collection of pathways, a single -omics assay, and a clinical response data frame. `SubsetPathwayData()` can extract the pathway-specific assay values and responses for a given pathway.
- `AESPCA_pVals()` & `SuperPCA_pVals()` execute the respective analysis on an `Omics*` object created by `CreateOmics()`.
- `getPathPCLs()` & `getPathpVals()` extract components of the AESPCA or SuperPCA analysis results.

All other functions are helper functions for these groups.
...
- Many tools, including **tool1**^[ CITE1 ] and **tool2**^[ CITE2 ], ignore expert-based biological information contained in biological pathways.
- Other tools, such as **tool3**^[ CITE3 ] and **tool4**^[ CITE4 ], incorporate biological pathways *after* PCs are extracted from the entire data set. This is figuratively like putting a holiday feast into a blender and trying to taste the individual dishes: the PCs will extract global -omic signals while drowning out the effects of individual biological pathways.
